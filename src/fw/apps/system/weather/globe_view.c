...
static void framebuffer_draw_starfield(GBitmap *fb, GlobeView *view, GRect bounds, GRect clip_rect);
#ifdef CONFIG_TOUCH
...
static int globe_max_selector_index(GlobeView *view) {
  return (view && view->saved_entry_count > 0) ? view->saved_entry_count - 1 : 0;
}
...
static bool selected_city_is_valid(GlobeView *view, int city_index) {
  ...
  return e->has_coordinates || (e->is_current_location && view && view->has_current_location);
}
...
static int16_t saved_entry_latitude_e2(GlobeView *view, SavedLocationEntry *entry) {
  if (entry && entry->is_current_location && view && view->has_current_location) {
    return (int16_t)view->current_location_latitude_e2;
  }
  return entry ? entry->latitude_e2 : 0;
}

static int16_t saved_entry_longitude_e2(GlobeView *view, SavedLocationEntry *entry) {
  if (entry && entry->is_current_location && view && view->has_current_location) {
...
static int nearest_city_index_for_orientation(GlobeView *view) {
  ...
  for (int i = 0; i <= max_index; i++) {
    ...
    if (!saved_entry_globe_coords(view, entry, &candidate_latitude_e2, &candidate_longitude_e2)) {
      continue;
    }

    int32_t lat_delta = candidate_latitude_e2 - view->color_latitude_e2;
    int32_t lon_delta =
        shortest_longitude_delta_e2(view->color_longitude_e2, candidate_longitude_e2);
  ...
  if (view->bw_sequence) {
    ...
    if (!frame) {
      frame = gdraw_command_sequence_get_frame_by_index(
          view->bw_sequence, bw_frame_index_for_view(view, view->current_frame));
    }
    if (frame) {
...
static AnimationProgress ease_in_out(AnimationProgress progress) {
  ...
  return (AnimationProgress)weather_scale_i32(shape, square, ANIMATION_NORMALIZED_MAX);
}
...
static int crumple_delay_index(GPoint point, GPoint center) {
  ...
  return (int)weather_scale_i32(distance_from_sweep, GLOBE_CRUMPLE_DELAY_SEGMENTS,
                                TRIG_MAX_ANGLE / 2);
}
...
static bool draw_direct_crumple_command(GDrawCommand *command, uint32_t index, void *context) {
  ...
  if (type == GDrawCommandTypeCircle) {
    ...
    uint16_t scaled_radius = (uint16_t)((uint32_t)radius * (ANIMATION_NORMALIZED_MAX - progress) /
    ...
    center = GPoint(center.x + crumple->origin.x, center.y + crumple->origin.y);
  ...
  if (!gdraw_command_get_path_open(command) && !gcolor_equal(fill_color, GColorClear)) {
    graphics_context_set_fill_color(crumple->ctx, fill_color);
...
static bool color_highlight_bw_command(GDrawCommand *command, uint32_t index, void *context) {
  (void)context;
...
#else
  gdraw_command_set_fill_color(command, index == 0 ? GColorVividCerulean : GColorScreaminGreen);
#endif
  return true;
}
...
static bool restore_bw_command(GDrawCommand *command, uint32_t index, void *context) {
  (void)index;
  (void)context;
  gdraw_command_set_stroke_color(command, GColorBlack);
...
static void draw_intro_world_frame(GContext *ctx, GlobeView *view, GPoint origin,
                                   GSize frame_size) {
...
static void draw_cradle(GContext *ctx, GlobeView *view, GPoint origin, GSize frame_size) {
  ...
  graphics_fill_radial(ctx, GRect(origin.x + acx - aor, origin.y + acy - aor, aor * 2, aor * 2),
                       GOvalScaleModeFitCircle, ath, GLOBE_CRADLE_ARM_START_ANGLE,
...
static int32_t trig_sin_q10(int angle) {
  return (int32_t)(sin_lookup(positive_modulo(angle, TRIG_MAX_ANGLE)) * GLOBE_ROT_SCALE /
                   TRIG_MAX_RATIO);
}

static int32_t trig_cos_q10(int angle) {
  return (int32_t)(cos_lookup(positive_modulo(angle, TRIG_MAX_ANGLE)) * GLOBE_ROT_SCALE /
                   TRIG_MAX_RATIO);
}
...
static void matrix_multiply(int32_t destination[9], const int32_t left[9], const int32_t right[9]) {
  int32_t result[9];
...
static void matrix_from_lat_lon(int32_t matrix[9], int32_t latitude_e2, int32_t longitude_e2) {
...
static void set_color_orientation(GlobeView *view, int32_t latitude_e2, int32_t longitude_e2) {
  ...
  view->starfield_offset_x_q8 = (int32_t)(((int64_t)positive_modulo(longitude_e2, 36000) *
                                           GLOBE_STARFIELD_WIDTH * GLOBE_Q8_ONE) /
                                          36000);
  view->starfield_offset_y_q8 = (latitude_e2 * GLOBE_STARFIELD_HEIGHT * GLOBE_Q8_ONE) / 36000;
#endif
}
  ...
  if (!view->bw_sequence) {
    ...
    if (view->bw_sequence) {
      ...
      if (view->bw_sequence_duration_ms == 0) {
        view->bw_sequence_duration_ms = view->bw_frame_count * GLOBE_FRAME_INTERVAL_MS;
      }
    }
  }

  if (!view->cradle_pdc) {
    view->cradle_pdc = gdraw_command_image_create_with_resource(RESOURCE_ID_GLOBE_CRADLE_PDC);
  }
  ...
  if (view->bw_sequence && !view->bw_sequence_scaled) {
    ...
    if (from.w > 0 && from.h > 0) {
      ...
      for (uint32_t i = 0; i < frames; i++) {
        GDrawCommandFrame *frame = gdraw_command_sequence_get_frame_by_index(view->bw_sequence, i);
        if (frame) {
          gdraw_command_list_scale(gdraw_command_frame_get_command_list(frame), from, to);
        }
      }
...
static inline uint8_t cubemap_sample(const uint8_t *cubemap_data, int32_t x, int32_t y, int32_t z) {
  int32_t ax = x < 0 ? -x : x;
  int32_t ay = y < 0 ? -y : y;
  int32_t az = z < 0 ? -z : z;
  int face;
  ...
  for (int ay = clipped.origin.y; ay < bottom; ay++) {
    ...
    for (int ax = row_left; ax <= row_right; ax++) {
      if (GLOBE_SPACE_FADE_DITHER[ay % GLOBE_SPACE_FADE_DITHER_SIZE]
                                 [ax % GLOBE_SPACE_FADE_DITHER_SIZE] < coverage) {
...
static GRect revealed_globe_layer_frame(GRect bounds) {
  int extent = revealed_globe_radius() + GLOBE_ATMOSPHERE_GLOW_PX + GLOBE_CITY_BOUNCE_PX +
               GLOBE_ATMOSPHERE_LAYER_MARGIN_PX;
  GPoint center = revealed_globe_center_for_bounds(bounds, 0);
  return clip_rect_to_bounds(
      GRect(center.x - extent, center.y - extent, (extent * 2) + 1, (extent * 2) + 1), bounds);
}
...
#else
      memset(&ri.data[row_left], GColorBlackARGB8, (size_t)(row_right - row_left + 1));
#endif
    }
  }
}

static void framebuffer_draw_starfield(GBitmap *fb, GlobeView *view, GRect bounds,
                                       GRect clip_rect) {
  if (!fb || !view || !view->starfield_data || view->starfield_size <= 2 || bounds.size.w <= 0 ||
      bounds.size.h <= 0) {
    return;
  }
...
#ifdef CONFIG_TOUCH
  int offset_x =
      positive_modulo((int)(view->starfield_offset_x_q8 / GLOBE_Q8_ONE), GLOBE_STARFIELD_WIDTH);
  int offset_y =
      positive_modulo((int)(view->starfield_offset_y_q8 / GLOBE_Q8_ONE), GLOBE_STARFIELD_HEIGHT);
#else
  ...
  for (uint16_t i = 0; i < count; i++) {
    ...
    int first_x = positive_modulo(source_x - (offset_x * mult), GLOBE_STARFIELD_WIDTH);
    int first_y = positive_modulo(source_y - (offset_y * mult), GLOBE_STARFIELD_HEIGHT);
    for (int local_y = first_y; local_y < bounds.size.h; local_y += GLOBE_STARFIELD_HEIGHT) {
      for (int local_x = first_x; local_x < bounds.size.w; local_x += GLOBE_STARFIELD_WIDTH) {
        int ax = bounds.origin.x + local_x;
        int ay = bounds.origin.y + local_y;
        if (ay < clip_top || ay > clip_bottom || ax < clip_left || ax > clip_right) {
          continue;
        }
...
static void framebuffer_fill_circle(GBitmap *fb, GPoint center, int radius, uint8_t color,
                                    GRect clip_rect) {
...
static PBL_NOINLINE void framebuffer_draw_ring(GBitmap *fb, GPoint center, int outer_r, int inner_r,
  ...
  for (int ay = center.y - outer_r; ay <= center.y + outer_r; ay++) {
    ...
    for (int ax = center.x - outer_r; ax <= center.x + outer_r; ax++) {
      if (ax < clip_left || ax > clip_right || ax < (int)ri.min_x || ax > (int)ri.max_x) {
        continue;
      }
      int dx = ax - center.x;
      int d2 = (dx * dx) + (dy * dy);
...
typedef struct {
  uint8_t outline_r, head_r, base_r, raise;
} GlobePinStyle;
...
static void framebuffer_draw_raised_pin(GBitmap *fb, GPoint globe_center, GPoint surface,
                                        int depth_q8, bool emphasized, GRect clip_rect) {
  int cls = depth_q8 >= GLOBE_PIN_DEPTH_FRONT_Q8 ? 2 : (depth_q8 >= GLOBE_PIN_DEPTH_MID_Q8 ? 1 : 0);
...
static bool project_lat_lon_to_globe_point_with_depth(GlobeView *view, int32_t latitude_e2,
  ...
  int32_t sx =
      (int32_t)(((int64_t)view->globe_rotation[0] * wx + (int64_t)view->globe_rotation[3] * wy +
                 (int64_t)view->globe_rotation[6] * wz) >>
                GLOBE_ROT_SHIFT);
  int32_t sy =
      (int32_t)(((int64_t)view->globe_rotation[1] * wx + (int64_t)view->globe_rotation[4] * wy +
                 (int64_t)view->globe_rotation[7] * wz) >>
                GLOBE_ROT_SHIFT);
  int32_t sz =
      (int32_t)(((int64_t)view->globe_rotation[2] * wx + (int64_t)view->globe_rotation[5] * wy +
...
static int nearest_centered_city_index(GlobeView *view, int radius_px) {
  ...
  for (int i = 0; i <= max_index; i++) {
    ...
    if (!saved_entry_globe_coords(view, entry, &entry_latitude_e2, &entry_longitude_e2)) {
      continue;
    }
...
static void settle_city_pin_to_center(GlobeView *view, int city_index) {
  int deadzone_sq = GLOBE_LOCK_SETTLE_DEADZONE_PX * GLOBE_LOCK_SETTLE_DEADZONE_PX;
...
static void draw_saved_location_pins(GBitmap *fb, GlobeView *view, GPoint center, int radius,
  ...
  for (int i = 0; i <= max_index && n < (int)(sizeof(pins) / sizeof(pins[0])); i++) {
    ...
    GPoint pin;
    int depth = 255;
    if (!project_lat_lon_to_globe_point_with_depth(
            view, saved_entry_latitude_e2(view, entry), saved_entry_longitude_e2(view, entry),
            center, pin_radius, GLOBE_PIN_EDGE_FRONT_Q10, &pin, &depth)) {
      continue;
    }
    ...
    if (view->city_anim && view->hover_lock_active) {
      ...
      emphasized = emphasized && view->city_anim_progress > ANIMATION_NORMALIZED_MAX / 2;
    }
#endif
    pins[n++] = (PinDrawEntry){.pt = pin, .depth = (uint8_t)depth, .emphasized = emphasized};
  }
  ...
  if (view->has_current_location && n < (int)(sizeof(pins) / sizeof(pins[0]))) {
    ...
    int depth = 255;
    if (project_lat_lon_to_globe_point_with_depth(
            view, view->current_location_latitude_e2, view->current_location_longitude_e2, center,
            pin_radius, GLOBE_PIN_EDGE_FRONT_Q10, &pin, &depth)) {
      pins[n++] = (PinDrawEntry){.pt = pin, .depth = (uint8_t)depth, .emphasized = false};
    }
  }
  ...
  for (int k = 0; k < n; k++) {
    framebuffer_draw_raised_pin(fb, center, pins[k].pt, pins[k].depth, pins[k].emphasized,
                                clip_rect);
  }
}
...
static inline bool prv_bw_lit(uint8_t argb8, int x, int y) {
  ...
  const int lum = ((argb8 >> 4) & 0x3) * 3 + ((argb8 >> 2) & 0x3) * 4 + (argb8 & 0x3) * 2; // 0..27
...
static int fill_globe_ring_row(uint8_t *row_data, int y, int cx, int dy_sq, const int32_t *radii_sq,
                               int lo, int hi) {
  ...
  static const uint8_t ring_colors[GLOBE_RING_COUNT] = {
    ...
    GColorBlueMoonARGB8, GColorVividCeruleanARGB8, GColorPictonBlueARGB8,
  ...
  for (int r = 0; r < GLOBE_RING_COUNT; r++) {
    fill_row_span(row_data, y, cx - spans[r], cx - spans[r + 1] - 1, lo, hi, ring_colors[r]);
    fill_row_span(row_data, y, cx + spans[r + 1] + 1, cx + spans[r], lo, hi, ring_colors[r]);
  }
...
static void draw_cubemap_globe_at_center(GContext *ctx, GlobeView *view, GPoint center,
  ...
  const int32_t ring_radii_sq[GLOBE_RING_COUNT + 1] = {
    ...
    (radius + 8) * (radius + 8),           (radius + 6) * (radius + 6), (radius + 4) * (radius + 4),
  ...
  for (int dy = -glow_outer_radius; dy <= glow_outer_radius; dy++) {
    ...
    const uint8_t *dither_row = GLOBE_SPACE_FADE_DITHER[ay % GLOBE_SPACE_FADE_DITHER_SIZE];
    ...
    for (int d = 0; d <= d_max; d++) {
      ...
      for (int side = 0; side < 2; side++) {
        ...
        if (sz < GLOBE_HAZE_RIM_SZ_Q10) {
          ...
          int32_t rim_light = (light_base + light_col) >> GLOBE_ROT_SHIFT;
        ...
        if (sz < GLOBE_HAZE_BLEND_SZ_Q10) {
          ...
          if (((dith * haze_span) >> 4) >= (sz - GLOBE_HAZE_RIM_SZ_Q10)) {
  ...
  if (view->lock_pulse_anim) {
    ...
    int outer_r =
        GLOBE_LOCK_PULSE_R_MIN_PX + (GLOBE_LOCK_PULSE_R_GROW_PX * eased) / ANIMATION_NORMALIZED_MAX;
    framebuffer_draw_ring(fb, GPoint(center.x, center.y - 5), outer_r,
                          outer_r - GLOBE_LOCK_PULSE_THICKNESS_PX, GColorCelesteARGB8, clipped);
  }
...
static void draw_cubemap_globe(GContext *ctx, GlobeView *view, GPoint origin, GSize frame_size,
  ...
  draw_cubemap_globe_at_center(ctx, view, center, scale_percent, clip_rect, clear_clip);
}
...
static AnimationProgress color_transition_grow_progress(int amount) {
  const int grow_start = (ANIMATION_NORMALIZED_MAX * GLOBE_COLOR_GROW_START_PERCENT) / 100;
  if (amount <= grow_start) {
    return 0;
  }

  return (AnimationProgress)weather_scale_i32(amount - grow_start, ANIMATION_NORMALIZED_MAX,
                                              ANIMATION_NORMALIZED_MAX - grow_start);
}
...
static int color_transition_scale_percent(int amount) {
  ...
  return weather_scale_i32(eased, GLOBE_COLOR_FINAL_SCALE_PERCENT, ANIMATION_NORMALIZED_MAX);
}

static int transition_color_amount(GlobeView *view) {
  int progress = (int)view->reveal_progress;
  return view->reveal_direction > 0 ? progress : (ANIMATION_NORMALIZED_MAX - progress);
}

static int transition_bw_amount(GlobeView *view) {
  int amount = transition_color_amount(view);
  if (view && view->reveal_direction > 0) {
    amount = (amount * GLOBE_REVEAL_BW_SPEED_NUM) / GLOBE_REVEAL_BW_SPEED_DEN;
...
static int bounce_offset(GlobeView *view) {
  ...
  return (view->bounce_direction * GLOBE_CITY_BOUNCE_PX * peak * 2) / ANIMATION_NORMALIZED_MAX;
}
...
static void notify_city_selected(GlobeView *view, bool force) {
  ...
  if (entry && view->location_select_callback) {
    view->location_select_callback(entry->ds_index, force, view->location_select_context);
  }
}
...
static bool commit_hovered_location(GlobeView *view) {
  ...
  if (view->is_free_roam) {
    int city_index = nearest_centered_city_index(view, GLOBE_COMMIT_LOCK_RADIUS_PX);
    if (city_index < 0) {
      mark_dynamic_globe_dirty(view);
      return false;
    }
    ...
    set_color_orientation(view, selected_city_latitude_e2(view), selected_city_longitude_e2(view));
...
static void format_selected_label(GlobeView *view, char *buffer, size_t buffer_size) {
  ...
  if (first_comma) {
    ...
    if (country_len == 13 && strncmp(country_start, "United States", 13) == 0)
      ...
      abbr = "UK";
    else if (country_len == 9 && strncmp(country_start, "Australia", 9) == 0)
      abbr = "AUS";
    else if (country_len == 20 && strncmp(country_start, "United Arab Emirates", 20) == 0)
      abbr = "UAE";
    else if (country_len == 12 && strncmp(country_start, "South Africa", 12) == 0)
      abbr = "SA";
    ...
    if (city_len > 0 && (abbr || country_len > 0)) {
      if (abbr) {
        ...
        snprintf(buffer, buffer_size, "%.*s, %.*s", (int)city_len, label, (int)country_len,
                 country_start);
      }
...
static void set_free_roam_enabled(GlobeView *view, bool enabled) {
  ...
  if (view->is_revealed && !view->is_revealing) {
    ...
    set_text_layer_hidden(view->city_label_layer, enabled && !view->hover_lock_active);
    mark_dynamic_globe_dirty(view);
  }
}
#endif
...
static void show_revealed_space_layers(GlobeView *view) {
  ...
  if (view->globe_layer) {
    Layer *root_layer = window_get_root_layer(view->window);
    layer_set_frame(view->globe_layer, revealed_globe_layer_frame(layer_get_bounds(root_layer)));
    layer_set_hidden(view->globe_layer, false);
    layer_mark_dirty(view->globe_layer);
  }
#ifdef CONFIG_TOUCH
  set_text_layer_hidden(view->city_label_layer, view->is_free_roam && !view->hover_lock_active);
#else
...
static void city_anim_update(Animation *anim, AnimationProgress progress) {
  ...
  if (view->hover_lock_active && view->hover_city_index >= 0) {
    ...
    int target_dx =
        weather_scale_i32(view->city_anim_longitude_delta_e2, remaining, ANIMATION_NORMALIZED_MAX);
    int target_dy =
        weather_scale_i32(view->city_anim_latitude_delta_e2, remaining, ANIMATION_NORMALIZED_MAX);
    int current_dx;
    int current_dy;
    if (city_pin_offset_from_center(view, view->hover_city_index, &current_dx, &current_dy)) {
      int correction_x = target_dx - current_dx;
      int correction_y = target_dy - current_dy;
      if (correction_x || correction_y) {
        apply_globe_screen_delta_q8(view, (int32_t)correction_x * GLOBE_Q8_ONE,
  ...
  int32_t latitude_e2 =
      ...
      weather_scale_i32(view->city_anim_latitude_delta_e2, eased, ANIMATION_NORMALIZED_MAX);
  int32_t longitude_e2 =
      view->city_anim_start_longitude_e2 +
      weather_scale_i32(view->city_anim_longitude_delta_e2, eased, ANIMATION_NORMALIZED_MAX);
...
static const AnimationImplementation s_city_anim_impl = {.update = city_anim_update};
...
static const AnimationImplementation s_bounce_anim_impl = {.update = bounce_anim_update};
...
static void start_lock_pulse(GlobeView *view) {
  ...
  view->lock_pulse_anim =
      start_view_animation(view, GLOBE_LOCK_PULSE_DURATION_MS, AnimationCurveLinear,
                           &s_lock_pulse_anim_impl, lock_pulse_anim_stopped);
...
static Animation *start_view_animation(GlobeView *view, uint32_t duration_ms, AnimationCurve curve,
  ...
  animation_set_handlers(anim, (AnimationHandlers){.stopped = stopped}, view);
...
static void start_city_bounce(GlobeView *view, bool is_down) {
  ...
  view->bounce_progress = 0;
  view->bounce_anim =
      start_view_animation(view, GLOBE_CITY_BOUNCE_DURATION_MS, AnimationCurveEaseOut,
                           &s_bounce_anim_impl, bounce_anim_stopped);
}
...
static void start_city_rotation(GlobeView *view, int city_index) {
  ...
  view->city_anim_latitude_delta_e2 =
      view->city_anim_target_latitude_e2 - view->city_anim_start_latitude_e2;
  view->city_anim_longitude_delta_e2 = shortest_longitude_delta_e2(
      view->city_anim_start_longitude_e2, view->city_anim_target_longitude_e2);
  view->city_anim_progress = 0;

  if (view->city_anim_longitude_delta_e2 == 0 && view->city_anim_latitude_delta_e2 == 0) {
    update_city_label_layer(view);
    mark_dynamic_globe_dirty(view);
    return;
  }
  ...
  view->city_anim =
      start_view_animation(view, GLOBE_CITY_ROTATION_DURATION_MS, AnimationCurveLinear,
                           &s_city_anim_impl, city_anim_stopped);
  update_city_label_layer(view);
  mark_dynamic_globe_dirty(view);
}
...
static bool try_start_magnetic_city_lock_v(GlobeView *view, int radius_px, int32_t vx_q8,
  ...
  int start_dx = 0;
  int start_dy = 0;
  bool has_offset = city_pin_offset_from_center(view, city_index, &start_dx, &start_dy);
  int deadzone_sq = GLOBE_LOCK_SETTLE_DEADZONE_PX * GLOBE_LOCK_SETTLE_DEADZONE_PX;
  ...
  if (!has_offset || (start_dx * start_dx) + (start_dy * start_dy) <= deadzone_sq) {
  ...
  view->city_anim = start_view_animation(view, swoop_ms, AnimationCurveLinear, &s_city_anim_impl,
                                         city_anim_stopped);
  if (!view->city_anim) {
...
static void navigate_city(GlobeView *view, bool is_down) {
  ...
  if (view->is_free_roam) {
    ...
    view->selected_city_index =
        nearest_index >= 0 ? nearest_index : nearest_city_index_for_orientation(view);
    set_free_roam_enabled(view, false);
  }
#endif
...
static void apply_globe_screen_delta_q8(GlobeView *view, int32_t dx_q8, int32_t dy_q8) {
  ...
  view->color_longitude_e2 =
      normalize_longitude_e2(view->color_longitude_e2 - ((dx_q8 * 86) / GLOBE_Q8_ONE));
  view->color_latitude_e2 =
      clamp_latitude_e2(view->color_latitude_e2 - ((dy_q8 * 86) / GLOBE_Q8_ONE));
...
static void update_globe_drag(GlobeView *view, int16_t x, int16_t y) {
  ...
  view->touch_last_x = x;
  view->touch_last_y = y;
  view->touch_velocity_x_q8 =
      clamp_globe_velocity_q8(((view->touch_velocity_x_q8 * 3) + ((int32_t)dx * GLOBE_Q8_ONE)) / 4);
  view->touch_velocity_y_q8 =
      clamp_globe_velocity_q8(((view->touch_velocity_y_q8 * 3) + ((int32_t)dy * GLOBE_Q8_ONE)) / 4);
  ...
  if (total_adx > GLOBE_DRAG_AXIS_THRESHOLD || total_ady > GLOBE_DRAG_AXIS_THRESHOLD) {
...
static void globe_coast_timer_handler(void *context) {
  ...
  apply_globe_screen_delta_q8(
      view, (view->coast_velocity_x_q8 * GLOBE_COAST_STEP_NUM) / GLOBE_COAST_STEP_DEN,
      (view->coast_velocity_y_q8 * GLOBE_COAST_STEP_NUM) / GLOBE_COAST_STEP_DEN);

  view->coast_velocity_x_q8 =
      (view->coast_velocity_x_q8 * GLOBE_COAST_DECAY_NUM) / GLOBE_COAST_DECAY_DEN;
  view->coast_velocity_y_q8 =
      (view->coast_velocity_y_q8 * GLOBE_COAST_DECAY_NUM) / GLOBE_COAST_DECAY_DEN;

  int32_t speed = abs_i32(view->coast_velocity_x_q8) + abs_i32(view->coast_velocity_y_q8);
  if (speed <= GLOBE_COAST_SETTLE_SPEED_Q8 &&
      try_start_magnetic_city_lock_v(view, GLOBE_LOCK_RADIUS_PX, view->coast_velocity_x_q8,
                                     view->coast_velocity_y_q8)) {
    return;
  }
  ...
  view->coast_timer = app_timer_register(GLOBE_COAST_FRAME_MS, globe_coast_timer_handler, view);
}
...
static void reveal_anim_update(Animation *anim, AnimationProgress progress) {
  ...
  if (view->reveal_direction > 0) {
    int eased = ease_out_quad(progress);
    int32_t lon_delta = shortest_longitude_delta_e2(view->transition_color_start_longitude_e2,
                                                    view->transition_color_target_longitude_e2);
    int32_t lat_delta =
        view->transition_color_target_latitude_e2 - view->transition_color_start_latitude_e2;
    ...
    int32_t reveal_spin_e2 =
        weather_scale_i32(GLOBE_REVEAL_COLOR_SPIN_DEGREES_E2, spin_eased, ANIMATION_NORMALIZED_MAX);
    next_longitude_e2 = normalize_longitude_e2(base_longitude_e2 - reveal_spin_e2);
  } else {
    next_latitude_e2 = 0;
    next_longitude_e2 =
        normalize_longitude_e2(view->transition_color_start_longitude_e2 -
                               weather_scale_i32((int32_t)progress, GLOBE_REVEAL_SPIN_STEPS * 1500,
                                                 ANIMATION_NORMALIZED_MAX));
  }
...
static void reveal_anim_stopped(Animation *anim, bool finished, void *context) {
  ...
  if (owns_anim && finished) {
    ...
    if (view->is_revealed) {
      set_color_orientation(view, view->transition_color_target_latitude_e2,
                            view->transition_color_target_longitude_e2);
      ...
      set_color_orientation(view, 0, longitude_e2_for_bw_frame(landed_bw_frame));
...
static void toggle_reveal(GlobeView *view) {
  ...
  view->reveal_anim = start_view_animation(
      view,
      view->reveal_direction < 0 ? GLOBE_REVEAL_REVERSE_DURATION_MS : GLOBE_REVEAL_DURATION_MS,
      AnimationCurveEaseOut, &s_reveal_impl, reveal_anim_stopped);
}

static void schedule_frame_timer(GlobeView *view) {
  view->animation_timer =
      app_timer_register(GLOBE_FRAME_INTERVAL_MS, animation_timer_handler, view);
}
...
static void note_globe_interaction(GlobeView *view) {
  ...
  view->bw_idle = false;
  view->bw_idle_slowdown_step = 0;
  if (view->is_animating && !view->is_revealed && !view->animation_timer) {
    schedule_frame_timer(view);
  }
  start_globe_idle_timer(view);
}
...
static void animation_timer_handler(void *context) {
  ...
  if (!view->is_revealing) {
    ...
    if (!view->bw_idle || view->bw_idle_slowdown_step < GLOBE_IDLE_SLOWDOWN_STEPS) {
      uint32_t duration = bw_sequence_duration_for_view(view);
      view->bw_elapsed_ms =
          duration > 0 ? (view->bw_elapsed_ms + GLOBE_FRAME_INTERVAL_MS) % duration : 0;
      view->current_frame =
          bw_frame_index_for_view(view, (int)(view->bw_elapsed_ms / GLOBE_FRAME_INTERVAL_MS));
      if (view->canvas_layer) {
        layer_mark_dirty(view->canvas_layer);
      }
    }
  ...
  if (view->bw_idle && !view->is_revealing) {
    ...
    uint32_t interval = GLOBE_FRAME_INTERVAL_MS +
                        (uint32_t)view->bw_idle_slowdown_step * GLOBE_IDLE_SLOWDOWN_STEP_MS;
    view->animation_timer = app_timer_register(interval, animation_timer_handler, view);
  } else {
    schedule_frame_timer(view);
  }
}
...
static void draw_transition_state(GContext *ctx, GlobeView *view, GPoint origin, GSize frame_size,
  ...
  draw_cubemap_globe(ctx, view, origin, frame_size, color_transition_scale_percent(color_amount),
                     bounds, false);

  if (!draw_bw_crumple_frame(ctx, view, origin, frame_size, bw_amount) && bw_amount <= 0) {
    draw_bw_frame(ctx, view, origin, frame_size);
  }
}
...
static GRect city_label_frame_for_bounds(GRect bounds) {
  ...
  return GRect(side, bounds.size.h - GLOBE_CITY_LABEL_HEIGHT - GLOBE_CITY_LABEL_ROUND_BOTTOM_INSET,
               bounds.size.w - (side * 2), GLOBE_CITY_LABEL_HEIGHT);
}
...
static void draw_saved_locations_pin(GContext *ctx, GPoint origin, GColor color, GColor bg_color) {
  static const GPoint kPinBody[] = {
    {8, 1},   {10, 1},  {11, 2},  {13, 3},  {15, 5}, {16, 7}, {16, 10}, {15, 11}, {15, 13},
    {14, 14}, {13, 16}, {10, 19}, {10, 20}, {9, 21}, {8, 20}, {8, 19},  {5, 16},  {4, 14},
    {3, 13},  {3, 11},  {2, 10},  {2, 7},   {3, 5},  {5, 3},  {7, 2},
  };
...
static void draw_saved_locations_label(GContext *ctx, GlobeView *view, GRect bounds,
                                       bool selected) {
#if !PBL_ROUND
...
static void canvas_layer_draw(Layer *layer, GContext *ctx) {
  ...
  if (view->is_revealing) {
    ...
    draw_cubemap_globe(ctx, view, GPoint(x, y + offset), frame_size,
                       GLOBE_COLOR_FINAL_SCALE_PERCENT, bounds, false);
    ...
    draw_saved_locations_label(ctx, view, bounds, !view->intro_world_selected);
  }
}
...
static void globe_layer_draw(Layer *layer, GContext *ctx) {
  ...
  GPoint center = revealed_globe_center_for_bounds(root_bounds, bounce_offset(view));
  draw_cubemap_globe_at_center(ctx, view, center, GLOBE_COLOR_FINAL_SCALE_PERCENT, frame, true);
}
...
static bool point_is_on_intro_globe(GlobeView *view, int16_t x, int16_t y) {
  ...
  int origin_y = ((bounds.size.h - frame_size.h) / 2) + GLOBE_CRADLE_CENTER_Y_OFFSET;
...
static bool point_is_on_saved_locations_label(GlobeView *view, int16_t y) {
  ...
  return y >= (bounds.size.h - GLOBE_SAVED_LABEL_HEIGHT - GLOBE_SAVED_LABEL_ROUND_BOTTOM_INSET);
}
...
static void touch_handler(const TouchEvent *event, void *context) {
  ...
  if (event->type == TouchEvent_Touchdown) {
    ...
    if (view->is_revealed && !view->is_revealing) {
      bool whole_screen =
          view->is_free_roam || view->coast_active || view->coast_timer || view->city_anim;
      begin_globe_drag_capture(view, event->x, event->y, whole_screen);
    }

  } else if (event->type == TouchEvent_PositionUpdate) {
    if (view->is_revealed && !view->is_revealing && !view->touch_active &&
        (view->is_free_roam || view->coast_active || view->coast_timer || view->city_anim)) {
      view->touch_active = true;
    ...
    if (view->is_revealed && !view->is_revealing && view->touch_controls_globe) {
      ...
      bool was_globe_tap =
          view->touch_down_on_globe && adx <= GLOBE_TAP_THRESHOLD && ady <= GLOBE_TAP_THRESHOLD;
      view->touch_controls_globe = false;
      view->touch_down_on_globe = false;
    ...
    if (adx <= GLOBE_TAP_THRESHOLD && ady <= GLOBE_TAP_THRESHOLD) {
      ...
      if (!view->is_revealed) {
        if (point_is_on_intro_globe(view, event->x, event->y)) {
          toggle_reveal(view);
        } else if (!view->is_revealing && point_is_on_saved_locations_label(view, event->y) &&
                   view->saved_locations_callback) {
          view->saved_locations_callback(view->saved_locations_context);
        }
      }
    } else if (!view->is_revealed && !view->is_revealing && adx > ady && dx > 0) {
...
GlobeView *globe_view_create(void) {
  ...
  *(GlobeView **)layer_get_data(view->space_layer) = view;
  ...
  view->globe_layer =
      layer_create_with_data(revealed_globe_layer_frame(bounds), sizeof(GlobeView *));
  if (!view->globe_layer) {
  ...
  *(GlobeView **)layer_get_data(view->globe_layer) = view;
  ...
  view->city_label_layer = text_layer_create(city_label_frame_for_bounds(bounds));
  ...
  text_layer_set_font(view->city_label_layer, fonts_get_system_font(FONT_KEY_GOTHIC_18_BOLD));
  text_layer_set_text_alignment(view->city_label_layer, GTextAlignmentCenter);
  text_layer_set_overflow_mode(view->city_label_layer, GTextOverflowModeTrailingEllipsis);
...
static void globe_view_reload_saved_locations(GlobeView *view) {
  ...
  view->saved_entry_count =
      saved_locations_get_entries(view->saved_entries, SAVED_LOCATIONS_MAX_ENTRIES);
  ...
  if (view->saved_entry_count <= 0) {
    ...
    snprintf(view->saved_entries[0].label, sizeof(view->saved_entries[0].label), "%s",
             view->current_location_label[0] ? view->current_location_label
                                             : i18n_get("Current Location", view));
  }

  int next_index = had_previous_entry ? find_saved_entry_index(view, &previous_entry) : -1;
  if (next_index < 0) {
...
void globe_view_set_current_location(GlobeView *view, const char *label, int16_t latitude_e2,
  ...
  if (label && label[0]) {
    strncpy(view->current_location_label, label, sizeof(view->current_location_label) - 1);
...
void globe_view_push_slide_in_right(GlobeView *view) {
  if (!view || !view->canvas_layer) {
    globe_view_push_animated(view, false);
    return;
  }
...
static void prv_slide_out_right_stopped(Animation *anim, bool finished, void *context) {
  ...
  if (view->canvas_layer && view->window) {
    layer_set_frame(view->canvas_layer, layer_get_bounds(window_get_root_layer(view->window)));
  }
}
