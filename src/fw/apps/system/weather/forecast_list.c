...
typedef struct {
  ...
  void (*on_select_request_cb)(
      void *ctx); // SELECT on the resting main view -> open the condensed view
...
static uint32_t prv_official_weather_pdc_res(WeatherType t) {
  switch (t) {
    case WeatherType_Sun:
      return RESOURCE_ID_SUNNY_DAY_TINY;
    case WeatherType_PartlyCloudy:
      return RESOURCE_ID_PARTLY_CLOUDY_TINY;
    case WeatherType_CloudyDay:
      return RESOURCE_ID_CLOUDY_DAY_TINY;
    case WeatherType_LightRain:
      return RESOURCE_ID_LIGHT_RAIN_TINY;
    case WeatherType_HeavyRain:
      return RESOURCE_ID_HEAVY_RAIN_TINY;
    case WeatherType_LightSnow:
      return RESOURCE_ID_LIGHT_SNOW_TINY;
    case WeatherType_HeavySnow:
      return RESOURCE_ID_HEAVY_SNOW_TINY;
    case WeatherType_RainAndSnow:
      return RESOURCE_ID_RAINING_AND_SNOWING_TINY;
    default:
      return RESOURCE_ID_GENERIC_WEATHER_TINY;
  }
}
...
static uint32_t prv_official_weather_pdc_res_small(WeatherType t) {
  switch (t) {
    case WeatherType_Sun:
      return RESOURCE_ID_SUNNY_DAY_SMALL;
    case WeatherType_PartlyCloudy:
      return RESOURCE_ID_PARTLY_CLOUDY_SMALL;
    case WeatherType_CloudyDay:
      return RESOURCE_ID_CLOUDY_DAY_SMALL;
    case WeatherType_LightRain:
      return RESOURCE_ID_LIGHT_RAIN_SMALL;
    case WeatherType_HeavyRain:
      return RESOURCE_ID_HEAVY_RAIN_SMALL;
    case WeatherType_LightSnow:
      return RESOURCE_ID_LIGHT_SNOW_SMALL;
    case WeatherType_HeavySnow:
      return RESOURCE_ID_HEAVY_SNOW_SMALL;
    case WeatherType_RainAndSnow:
      return RESOURCE_ID_RAINING_AND_SNOWING_SMALL;
    default:
      return RESOURCE_ID_GENERIC_WEATHER_SMALL;
  }
}
#endif
...
static void prv_format_conditions(void) {
  for (int i = 0; i < (int)s_list->num_days; i++) {
    ...
    const char *phrase =
        (i == 0) ? f->current_phrase_now // current-hour phrase, both shapes (falls back to synced)
...
// precipitation view's icon row lands PIXEL-IDENTICAL to its pre-spread layout (the spread applies
// to the mainscreen only). On rect both pairs are equal -> the lerp is a no-op.
...
#define R5_BOW_DY(i, n, ss) \
  ((int)s_round_col_bow[(i) < (n) / 2 ? (n) / 2 - (i) : (i) - (n) / 2] * (ss) / R5_SECTION_TRAVEL)
#else
...
#endif
  weather_render_squash(ctx, s_list->squash_scratch, s_list->squash_in_p, s_list->squash_mode);
}
...
static void prv_start_up_to_card(void) {
  ...
  // Must match expanded_view.c's static card icon — shared constants keep the fly landing
  // pixel-exact.
...
static void prv_start_header_transition(bool to_scrolled) {
  ...
  s_list->anim = prv_start_anim(
      s_list->header_hasted ? interpolate_moook_soft_duration(3) : interpolate_moook_duration(),
      AnimationCurveLinear, &s_header_anim_impl, prv_header_anim_stopped);
  s_list->fx_active = true;
...
static void prv_clock_stage1_update(Animation *anim, AnimationProgress progress) {
  ...
  AnimationProgress pm =
      (progress > ANIMATION_NORMALIZED_MAX / 2) ? ANIMATION_NORMALIZED_MAX : progress * 2;
  ...
  s_list->fx_dot_y =
      R5_DOT_REST_Y + (R5_SCREEN_CY - R5_DOT_REST_Y) * eased / ANIMATION_NORMALIZED_MAX;
  layer_mark_dirty(s_list->canvas);
}
...
static void prv_start_clock_stage2(void) {
  ...
  s_list->clock_anim =
      prv_start_anim(220, AnimationCurveLinear, &s_clock_stage2_impl, prv_clock_stage2_stopped);
}
...
static void prv_start_clock_stage1(void) {
  ...
  s_list->clock_anim =
      prv_start_anim(interpolate_moook_soft_duration(PBL_IF_ROUND_ELSE(10, 3)),
...
static void prv_draw_unfolding_paper(GContext *ctx) {
  ...
  gdraw_command_list_scale_stroke_width(
      list, Fixed_S16_3((2 * R5_REPORT_BALL_R) << FIXED_S16_3_PRECISION), FIXED_S16_3_ONE,
...
static void prv_draw_exiting_paper(GContext *ctx) {
  ...
  if (!s_list->fly_lookup) { // stage-3's angle fan was freed at arm; build the exit's
    GPointIndexLookup *lu =
        gdraw_command_list_create_index_lookup_by_distance(list, GPoint(nsz.w, nsz.h / 2));
    if (!lu) {
      gdraw_command_image_destroy(clone);
      return;
    }
  ...
  gdraw_command_image_scale_segmented_to(clone, from, to, s_list->report_p, prv_moook_full,
                                         s_list->fly_lookup,
...
// move, exactly like Timeline), then draw the clone at its END position `end_origin`. The
// persistent s_list icons are never touched. `travel` = how far up the icon has come
// (R5_SECTION_TRAVEL for the 5-day discs, R5_HEADER_TRAVEL for the today bitmap). Returns true if
// it drew the deformed clone.
//
...
static void prv_draw_flying_icon(GContext *ctx) {
  ...
  gdraw_command_list_scale_segmented_to(list, size, intermediate, to, seg1, prv_moook_soft3, lookup,
                                        R5_FX_POINT_DURATION, true);
...
static void prv_draw_flying_content(GContext *ctx) {
  ...
  expanded_view_draw_glance_content(ctx, W, tdx, updated, s_list->fly_sunset, s_list->fly_temp,
                                    s_list->fly_uv, s_list->fly_precip, s_list->fly_wind, s_list);
}
...
static void prv_draw_stat_pill(GContext *ctx, const char *label, int y, int W, GColor fill) {
  ...
  // centring the line box reads low. Lift it by that asymmetry so the caps sit on the pill's
  // centre.
...
static void prv_draw_sun_body(GContext *ctx, GPoint c, int scale_pct, GColor outline_col) {
  ...
  GPoint pts[8] = {
    {(int16_t)-d, (int16_t)-a}, {(int16_t)-a, (int16_t)-d}, {(int16_t)-a, (int16_t)d},
    {(int16_t)-d, (int16_t)a},  {(int16_t)d, (int16_t)a},   {(int16_t)a, (int16_t)d},
...
static const uint16_t kSunRayAngles[10] = {
  // the PDC's own ray positions (from top, cw)
  0,
...
static void prv_draw_animated_sun(GContext *ctx, GPoint c, int32_t phase, int scale_pct) {
  ...
  for (int i = 0; i < R5_SUN_RAYS; i++) {
    ...
    const GPoint p0 = GPoint(prv_sun_proj(inner10, sin_a, c.x), prv_sun_proj(inner10, -cos_a, c.y));
...
static const RainDrop kStormDrops[4] = {
  ...
  {-4, 19, 8},
  {2, 13, 4},
  {8, 17, 11}, // centred under the cloud
};
...
static void prv_hide_rain_proc(GDrawCommandProcessor *processor, GDrawCommand *pc, size_t max_size,
                               const GDrawCommandList *list, const GDrawCommand *command) {
...
static void prv_hide_sun_proc(GDrawCommandProcessor *processor, GDrawCommand *pc, size_t max_size,
                              const GDrawCommandList *list, const GDrawCommand *command) {
...
static void prv_draw_animated_partly_cloudy(GContext *ctx, GPoint pdc_offset, int32_t phase) {
  ...
  prv_draw_animated_sun(
      ctx, GPoint(pdc_offset.x + 33 * R5_TODAY_ICON / 50, pdc_offset.y + 16 * R5_TODAY_ICON / 50),
...
static void prv_cloud_hide_proc(GDrawCommandProcessor *processor, GDrawCommand *pc, size_t max_size,
                                const GDrawCommandList *list, const GDrawCommand *command) {
...
static void prv_draw_animated_cloudy(GContext *ctx, GPoint pdc_offset, int32_t phase) {
  ...
  const int32_t aU =
      (int32_t)tick * (TRIG_MAX_ANGLE / 100) + 0x5000; // upper cloud, faster + offset
  ...
  gdraw_command_image_draw_processed(
      ctx, s_list->today_pdc, GPoint(pdc_offset.x + lx, pdc_offset.y + ly), &keep_lower.base);
  CloudHideProc keep_upper = {.base = {.command = prv_cloud_hide_proc}, .counter = 0, .keep = 1};
  gdraw_command_image_draw_processed(
      ctx, s_list->today_pdc, GPoint(pdc_offset.x + ux, pdc_offset.y + uy), &keep_upper.base);
}
...
static void prv_draw_animated_precip(GContext *ctx, GPoint pdc_offset, int32_t phase,
  ...
  for (int i = 0; i < st->n_drops; i++) {
    ...
    if (st->snow) {
      ...
      const int r = (prog <= peak) ? 1 + (R5_SNOW_MAX - 1) * prog / peak
...
static const PrecipStyle *const kFallStyles[] = {
  [WeatherType_LightSnow] = &kLightSnowStyle,  [WeatherType_LightRain] = &kLightRainStyle,
  [WeatherType_HeavyRain] = &kStormStyle,      [WeatherType_HeavySnow] = &kHeavySnowStyle,
  [WeatherType_RainAndSnow] = &kRainSnowStyle,
};

static void prv_draw_today_anim(GContext *ctx, const WeatherLocationForecast *today, int x, int y) {
  (void)today;
  ...
  switch (prv_header_type()) {
    case WeatherType_Sun:
      ...
      break;
    case WeatherType_PartlyCloudy:
      prv_draw_animated_partly_cloudy(ctx, GPoint(x, y), s_list->sun_phase);
      break;
    case WeatherType_CloudyDay:
...
static void prv_draw_series_links(GContext *ctx, const int *colx, const int *y, const bool *ok,
                                  int n, GColor c) {
...
static void prv_canvas_draw_round_5day(Layer *layer, GContext *ctx) {
  ...
  // and an unclamped lerp would extrapolate the pitch negative mid-transition.
  {
    ...
    const int step_high =
        R5_COL_STEP_HIGH - (R5_COL_STEP_HIGH - R5_COL_STEP_HIGH_SCROLLED) * ssc / R5_SECTION_TRAVEL;
    for (int i = 0; i < n; i++) {
  ...
  // on a 12.5-30fps path, so both live in caches keyed on the next-midnight timestamp.
  {
    ...
    if (now >= s_list->date_cache_expiry || s_list->today_header_dirty) {
      ...
      if (lt) {
        ...
        time_t to_midnight = (time_t)(86400 - (lt->tm_hour * 3600 + lt->tm_min * 60 + lt->tm_sec));
...
#if !PBL_ROUND
    const GSize dsz = graphics_text_layout_get_content_size(daydate, day_font, GRect(0, 0, W, 20),
  ...
  if (s_list->fx_active && fade <= 0 && s_list->today_pdc && !s_list->flying_icon) {
    ...
    const GPoint tend =
        GPoint(today_x, (s_list->header_to == R5_HEADER_TRAVEL) ? (R5_TODAY_Y - R5_HEADER_TRAVEL)
                                                                : R5_TODAY_Y);
  ...
  if (today_jellied) {
    ...
    if (fade > half) {
      ...
      prv_fade_erase(
          ctx, GRect(today_x - 10, R5_TODAY_Y - hs + drop, R5_TODAY_ICON + 20, R5_TODAY_ICON + 8),
          keep_out, icon_bg);
      ...
      prv_fade_erase(ctx,
                     GRect(today_x - 10, R5_TODAY_Y - hs + rise, R5_TODAY_ICON + 20, R5_TODAY_ICON),
                     keep_in, icon_bg);
    }
  }
  char tnow[16];
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (s_list->pdc_icons[icon_day]) {
      ...
      const int iend_cy =
          (s_list->header_to == R5_HEADER_TRAVEL) ? (R5_ICON_CY - R5_SECTION_TRAVEL) : R5_ICON_CY;
  ...
  // Cadet-Blue dots on a thin 1px link. No axes, no area fill.
  {
    ...
    // midpoint is the graph centre. Worked in section coords (+ss) so gshift is a constant the
    // whole fan still scrolls by. -1 corrects for the temp labels reaching ~2px further below the
    // low dot than above the high dot.
...
static void prv_canvas_draw(Layer *layer, GContext *ctx) {
  ...
  if (s_list->report_fx == 4) {
    ...
    const int dx = (int)prv_moook_full(s_list->report_p, 0, -layer_get_bounds(layer).size.w);
  ...
  for (int i = 0; i < (int)s_list->num_days; i++) {
    ...
    if (ry > LIST_TOP_Y) {
      graphics_draw_line(ctx, GPoint(day_label_x, ry), GPoint(W - right_margin, ry));
    }
    ...
    if (s_list->icons[i]) {
      ...
      weather_icon_draw(ctx, s_list->icons[i], GRect(icon_x, iy, ICON_SIZE, ICON_SIZE),
  ...
  if (max_s > 0) {
    ...
    graphics_fill_rect(ctx, GRect(W - side_inset - 3, bar_y, 3, bar_h), 0, GCornerNone);
  }
...
static void prv_click_up_down(ClickRecognizerRef recognizer, void *context) {
  ...
  if (down) {
    if (s_list->header_shown)
      prv_start_header_transition(true);
    else
      prv_start_clock_stage1(); // DOWN again when scrolled -> clock burst
  } else {
    if (!s_list->header_shown)
      prv_start_header_transition(false);
    else
      prv_start_up_to_card(); // UP at top -> squash + hero icon-fly to the card
  }
...
#endif
  // Fallback (scratch OOM on either shape): the legacy 115ms moook slide; done cb fires in its
  // .stopped.
#if PBL_ROUND
...
static void prv_click_select(ClickRecognizerRef recognizer, void *context) {
  ...
  if (!s_list->header_shown || s_list->fx_active || s_list->clock_fx || s_list->squash_mode ||
      s_select_exit_anim || s_list->report_fx)
    return;
#endif
    ...
    if (ady >= adx) {
      ...
      if (dy < 0) {
        if (s_list->header_shown)
          ...
          prv_start_clock_stage1();
      } else {
        if (!s_list->header_shown)
          prv_start_header_transition(false);
        else
          prv_start_up_to_card(); // BOTH shapes: the hero fly +
...
static void prv_list_raw_up_press(ClickRecognizerRef r, void *ctx) {
  prv_list_raw(BUTTON_ID_UP, true);
}
static void prv_list_raw_up_rel(ClickRecognizerRef r, void *ctx) {
  prv_list_raw(BUTTON_ID_UP, false);
}
static void prv_list_raw_down_press(ClickRecognizerRef r, void *ctx) {
  prv_list_raw(BUTTON_ID_DOWN, true);
}
static void prv_list_raw_down_rel(ClickRecognizerRef r, void *ctx) {
  prv_list_raw(BUTTON_ID_DOWN, false);
}
#endif
...
static bool prv_sun_step(void) {
  ...
  if (s_list->icon_fade > 0) {
    ...
    s_list->sun_phase += PBL_IF_ROUND_ELSE((33 * R5_SUN_ANGLE_STEP) / 20, 2 * R5_SUN_ANGLE_STEP);
  }
  if (s_list->canvas) {
    layer_mark_dirty(s_list->canvas);
  }
...
static void prv_window_load(Window *window) {
  ...
  s_list->header_scroll = 0;
  if (s_list->num_days > 0 &&
      (prv_header_type() == WeatherType_Sun || prv_header_type() == WeatherType_PartlyCloudy ||
       prv_header_type() == WeatherType_CloudyDay || prv_header_type() == WeatherType_LightRain ||
       prv_header_type() == WeatherType_HeavyRain || prv_header_type() == WeatherType_LightSnow ||
...
#if defined(CONFIG_TOUCH) && !PBL_ROUND
  if (s_list && !s_list->clock_loc_done && s_list->num_days > 0 && s_list->days[0].location_name &&
      s_list->days[0].location_name[0]) {
  ...
  if (s_list && s_pending_squash_mode) {
    ...
    if (s_pending_return_fly && m == SQUASH_RISE_IN && !s_list->flying_icon &&
      ...
      if (pdc) {
        const int W = layer_get_bounds(s_list->canvas).size.w;
        s_list->fly_pdc = pdc;
        s_list->fly_src = GRect((W - EV_ICON_SIZE) / 2, EV_ICON_Y, EV_ICON_SIZE, EV_ICON_SIZE);
        s_list->fly_dest = GRect(s_list->today_icon_x, R5_TODAY_Y, R5_TODAY_ICON, R5_TODAY_ICON);
        s_list->flying_icon = true;
      }
    }
...
void forecast_list_push(const WeatherLocationForecast *days, size_t num_days, int start_day_index,
  ...
  prv_forecast_list_push(days, num_days, start_day_index, animated, on_pop, on_pop_ctx,
                         on_city_select, on_city_select_ctx);
}
