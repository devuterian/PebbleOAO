...
const uint8_t weather_diurnal_curve[24] = {
  10, 6, 3, 0, 0, 3, 8, 16, 28, 42, 56, 70, 82, 92, 98, 100, 96, 88, 76, 62, 48, 36, 26, 17,
};
...
static void prv_squash_resample(GBitmap *fb, const uint8_t *scratch, AnimationProgress m,
                                int mode) {
...
int32_t weather_scale_i32(int32_t value, int32_t numerator, int32_t denominator) {
  ...
  uint32_t scaled = weather_scale_u32((uint32_t)value, (uint32_t)numerator, (uint32_t)denominator);
...
static void prv_fill_wrapped_radial(GContext *ctx, GRect rect, uint16_t inset, int32_t start,
                                    int32_t span) {
  ...
  if (end <= max) {
    ...
    graphics_fill_radial(ctx, rect, GOvalScaleModeFitCircle, inset, 0, end - max);
  }
}

void weather_draw_lava_ring(GContext *ctx, GPoint center, int outer_r, GColor glow_color,
                            uint32_t phase_u, uint8_t idle_progress) {
