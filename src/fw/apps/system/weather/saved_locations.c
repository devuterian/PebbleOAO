  ...
  for (int pass = 0; pass < 2 && count < max_entries; pass++) {
    for (int i = 0; i < total && count < max_entries; i++) {
      ...
      *e = (SavedLocationEntry){
        ...
        .has_coordinates = (ds->latitude_e2 != INT16_MIN && ds->longitude_e2 != INT16_MIN),
...
static void prv_draw_glance_row(GContext *ctx, const Layer *cell_layer,
  ...
  if (icon) {
    ...
    weather_icon_draw(ctx, icon, GRect(bounds.origin.x + 5, (bounds.size.h - 25) / 2, 25, 25),
...
static void prv_touch_handler(const TouchEvent *event, void *context) {
  ...
  if (event->type == TouchEvent_PositionUpdate && view->touch_active) {
    ...
    if (!view->drag_axis_set && (adx > SAVED_LOCATIONS_TOUCH_AXIS_THRESHOLD_PX ||
    ...
    if (view->drag_is_vertical) {
      // 1:1 finger tracking.
      int amount = prv_touch_clamp_scroll(view, view->scroll_at_drag_start - (int)dy);
