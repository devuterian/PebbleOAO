...
static void settings_bluetooth_toggle_airplane_mode(SettingsBluetoothData *data) {
  ...
  data->toggle_state = airplane_mode ? ToggleStateEnablingBluetooth : ToggleStateDisablingBluetooth;
...
static void settings_bluetooth_update_remotes_private(SettingsBluetoothData *data) {
  ...
  if (!data->remote_list_head) {
    ...
    sniprintf(
        data->header_buffer, HEADER_BUFFER_SIZE,
        (num_remotes != 1) ? i18n_get("%u Paired Phones", data) : i18n_get("%u Paired Phone", data),
        num_remotes);
  }
}
...
static void draw_stored_remote_item(GContext *ctx, const Layer *cell_layer, uint16_t device_index,
                                    SettingsBluetoothData *data) {
  const uint32_t num_remotes = list_count(data->remote_list_head);
  PBL_ASSERT(device_index < num_remotes, "Got index %" PRId16 " only have %" PRId32, device_index,
             num_remotes);
  ...
  if (row == 0) {
    ...
    if (data->toggle_state == ToggleStateIdle) {
      ...
      subtitle = (data->toggle_state == ToggleStateDisablingBluetooth)
                     ? i18n_get("Disabling...", data)
                     : i18n_get("Enabling...", data);
