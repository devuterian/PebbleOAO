...
아래는 **A부터 G까지 쌓인 공개 펌웨어 기능**입니다. 최신 프리릴리즈는 [G · Gelato](https://github.com/devuterian/PebbleOAO/releases/tag/v4.37.0-ver007-gelato), 최신 정식 릴리즈는 [E · Egg Salad](https://github.com/devuterian/PebbleOAO/releases/tag/v4.37.0-ver005-egg-salad)입니다. F와 G의 변경 사항까지 쓰시려면 프리릴리즈를 받아야 합니다요.
...
펌웨어 버전과 앱 버전은 따로 올라갑니다. **현재 F·G 릴리즈에 첨부된 APK는 ver011**입니다. 아래 ver016·ver017까지의 변경은 제 폰에 설치해서 확인한 작업본이고, 아직 그 릴리즈의 APK가 바뀐 건 아닙니다. 헷갈리실까 봐 적어둡니다요.

**공개 APK에 들어간 기능**

- **이 저장소의 펌웨어를 찾아줍니다.** 최신 정식 릴리즈를 확인하고, 프리릴리즈 받기를 켜면 시험 버전도 찾습니다. 처음 켤 때는 경고를 읽고 동의해야 합니다.
- **공식 업데이트 경로도 고를 수 있습니다.** 제 업데이트를 기다리기 싫어지시면 시계별로 공식 PebbleOS 경로로 바꿔두시면 됩니다.
- **업데이트 설명의 볼드·목록·사진·GIF를 보여줍니다.** 정확히 페블 화면 크기인 작은 스크린샷은 3배로 키워서 보여줍니다. 표 안 HTML 사진은 아래 ver017에서 따로 고쳤습니다.
- **한국어 음성 입력 연결과 결과 처리를 손봤습니다.** 서버 연결 설정을 바로잡고, 빈 결과나 불필요한 제어 문자가 시계로 넘어갈 때의 처리를 고쳤습니다. 마이크 음질이나 인식 성능 자체를 보장하는 기능은 아닙니다.
- **일출·일몰 알림도 한국어로 보냅니다.** 앞에서 적은 앱별 알림 아이콘과 배경색도 이 앱에서 보내줍니다.

**그 뒤로 작업한 것들 · 공개 APK 반영 전**
...
- **앱 자체 업데이트 확인을 넣었습니다.** ver016부터 GitHub의 새 APK를 자동으로 확인하고 알려줍니다. 설정에서 직접 확인할 수도 있습니다. 설치할 때는 안드로이드 확인 창을 거치며, 파일과 앱 서명·버전을 검사합니다. 이 기능이 들어간 앱을 처음 한 번은 설치해야 합니다.
- **폰 앱도 한국어로 바꿨습니다.** ver016부터 한국어 환경에서는 메뉴·버튼·권한 안내·연결 상태·건강 화면·휴대폰 알림을 한국어로 보여주고 시스템 글꼴을 씁니다. 외부 로그인 페이지나 배터리 웹 서비스의 원문, 사용자가 쓴 내용까지 번역하는 건 아닙니다.
